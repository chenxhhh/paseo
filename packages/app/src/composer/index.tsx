...
function ComposerContentImpl({
  ...
  const insertQuickCommandPrompt = useCallback(
    (prompt: string) => {
      const text = textSource.getSnapshot();
      const cursorIndex = Math.min(cursor.getState(), text.length);
      const insertion = buildQuickCommandInsertion({ text, prompt, cursorIndex });
