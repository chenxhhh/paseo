...
| Provider + ACP session unit tests                           | 118/118 passed incl. new `resumeAfterLoad` cases                                                                                                                                                                                                                                                                      |
...
Verified this phase: outgoing parameter transmission (via CLI DEBUG logs, no TLS weakening), closed-session restoration semantics (Paseo `resumeAfterLoad`), permission behavior (confirmed NOT routed through ACP). Remaining before normal activation: decide how to live with official CLI not consuming injected `mcpServers` (global `mcp_config.json` only), and broader regression (images, SSE MCP, history replay). Keep original providers available; the adapter remains experimental.
