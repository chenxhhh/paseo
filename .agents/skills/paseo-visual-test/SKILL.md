...
12. **`browser_evaluate` 派发合成事件后，React 18 不会在同一 task 内 flush 状态更新**——同一 evaluate 里 `el.click()` 后立刻读 DOM 断言必然拿到旧状态（症状：点击"生效与否"看起来是随机的）。事件派发和断言读回必须分成两次 `browser_evaluate` 调用。jsdom 单测里 `fireEvent` 被 act() 包裹才显得同步，真实浏览器没有这一层。
13. **行尾操作图标（kebab、行内归档图标）在 a11y snapshot 里没有 ref**——web 端它们 `accessibilityRole=undefined`，snapshot 整棵树都看不到（截图里明明在）。驱动方式：`browser_hover` hover 行（行本身有 ref）让图标挂载，然后 `browser_evaluate` 用 `[data-testid="sidebar-workspace-kebab-<key>"]` 之类的选择器找到节点，派发 `pointerdown`/`pointerup`/`click` 序列（裸 `el.click()` 也能触发 onPress，但序列更接近真实输入）。
14. **后台标签页节流毒化整套交互与截图**：Paseo 内置浏览器标签常驻后台（用户不点进去就从未合成）→ ① `browser_screenshot` 一直报 `screenshot_no_frame`；② 页面里所有定时器被钳到 ≥1s，Pressable 的长按窗口（500ms）被打乱——把 down/up 分在两次 evaluate 派发必出竞速错乱；③ 反复 HMR 后自动化通道可能整体卡死（evaluate 也超时），关 tab 重开即恢复。对策：交互时序放进**单次 async evaluate**内部完成（down→await→读状态→up）；视觉验证改用 `getComputedStyle(...).opacity/backgroundColor` 数值断言；一个加载世界里尽量跑完全部步骤。
15. **RN-web 合成事件挑组件**：FlatList 行的 Pressable 对合成 pointer/mouse/touch 一律不进 responder（疑与非可信事件无法 setPointerCapture + 列表滚动 responder 有关），而**portal 类容器里的按钮**（抽屉菜单项、Modal 底部弹层按钮）合成序列有效。要驱动行级 `onLongPress`：可信输入只有 Playwright `mouse.down()`；否则临时在源码加 window 测试钩子直接调 setState（如 `setActionAgent(agent)`），测完删干净。
16. **直达 `/sessions` 会被启动恢复守卫重定向**：localStorage 清过后没有 remembered workspace，深链 `/sessions` 与 `/h/<serverId>/sessions` 都会落到 `/open-project`。正确姿势：先 `localStorage.setItem("paseo:last-workspace-route-selection", JSON.stringify({serverId, workspaceId}))` 再导航到根路径让恢复逻辑进入 workspace，等 WS online 后**从应用内 SPA 点击「历史」**进入 History。注意不要用 document.title 或文本早判成功——标题含运行中 agent 名会假阳性。另有一个疑似上游 bug：冷挂载早于 WS 握手完成时，`useAgentHistory` 的 host 可达性判定不会随连接建立恢复（targets 永远空、`fetch_agent_history_request` 永远不发、界面永远 "无法加载历史"），SPA 内二次导航才正常——值得上游修。
17. **清理进程别按宽泛关键词扫**：`CommandLine -match "<worktree>"` 会把 Metro dev server（dev-app.sh 的 node，命令行引用 worktree 路径）一起杀掉。正确顺序：先 TaskStop 自己起的后台任务 → netstat 按 6768/8083 找残留 PID → 按 PID `taskkill /T /F` 点杀。
18. **`npm run dev:server` 不带 watch**（root 脚本链到 `node --import tsx`，不是 `tsx watch`）——改完 server 源码必须重启 daemon 任务才生效；判断活没活看监听 PID 是否变化。
19. **daemon 内部事件要 trace 级日志**：`PASEO_LOG_LEVEL=trace PASEO_LOG_FILE_LEVEL=trace npm run dev:server`。info 级什么 agent 事件都没有；trace 走 concurrently 控制台流（带 `[server]` 前缀和 ANSI 色），落在后台任务的 output 文件里，pino-pretty 格式 `[HH:MM:SS.mmm] LEVEL: msg {json}`，解析前先剥 ANSI。判定"事件来自哪条代码路径"时，在候选分支插临时 `logger.warn` 探针最快（测完撤掉）。
20. **模型跑得比浏览器往返快**：GLM-5.3-Flash 生成 30 项列表仅 ~25s，两次 evaluate 之间的回合可能已经结束，mid-stream 干预全空。时序敏感的操作（长输出中途发消息/取消）必须放进**单次 evaluate** 内：输入→轮询增长→发送→采样一条龙；automation 15s 超时没关系，页面脚本会继续跑完，隔一会儿再读状态即可。
21. **后台标签页的 UI 可能不渲染后续流式输出**（WS 事件到了、DOM 不更新，刷新后由持久化时间线补齐）——断言"回合有没有产出"别只看当时的 DOM，要么刷新重拉，要么查 provider 侧转录（CodeBuddy 在 `~/.codebuddy/projects/<编码cwd>/<sessionId>.jsonl`，含 user/assistant/`Interrupted by user` 等行，是权威事实源）。
